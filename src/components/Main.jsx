...
export default function Main() {
  ...
  useEffect(() => {
    const clearPreviousTimer = async () => {
      ...
      if (regNumber) {
        try {
          await api.delete(`${import.meta.env.VITE_REACT_APP_SERVER_HOSTNAME}/api/timer/${regNumber}`)
          localStorage.removeItem("lastRegistrationNumber")
        } catch (error) {
          console.log("Error clearing previous timer (non-critical):", error.message)
          localStorage.removeItem("lastRegistrationNumber")
  ...
  async function startQuiz(event) {
    ...
    if (Object.values(userData).every((value) => value)) {
      ...
      try {
        ...
        const response = await api.post(`${import.meta.env.VITE_REACT_APP_SERVER_HOSTNAME}/api/user`, userData)
        ...
        // Check for user already exists error (409 Conflict)
        if (err.response?.status === 409 && err.response.data?.isExisting) {
          setError(`${err.response.data.error} Switching to sign-in...`)
          ...
          setTimeout(() => {
            ...
            // Pre-fill the sign-in form with the email or registration number
            if (err.response.data.field === "email") {
              // The email is already registered, pre-fill it in the sign-in form
              ...
              // The registration number is already registered
              localStorage.setItem(
                "prefillSignIn",
                JSON.stringify({
