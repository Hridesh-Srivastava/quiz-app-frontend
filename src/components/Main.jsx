...
export default function Main() {
  ...
  async function startQuiz(event) {
    ...
    if (!/^[a-zA-Z0-9]+$/.test(userData.registrationNumber)) {
      setError("Invalid registration number format. Only alphabets and numerals are allowed.");
      return;
    }
  ...
  return (
    <div className="container">
      ...
      <form id="form" onSubmit={startQuiz}>
        ...
        <div>
          <input name="section" ref={sectionRef} className="userid" type="text" placeholder="Section" required />
        </div>
        <div className="start">
